...
In the following code, `data` is just an arbitrary **variable** name to hold the result of `read.table` and it can be called (or named) [*almost* anything](https://www.datamentor.io/r-programming/variable-constant/).
...
ggplot(df) +
  geom_jitter(aes(x = -log10(pvalue), y = -log10(adjusted.pvalue))) +
  theme_light()
```
...
df1 = df %>% mutate (value = pvalue, type = "Original")
df2 = df %>% mutate (value = adjusted.pvalue, type = "Adjusted" )
df3 = df1 %>% bind_rows(df2)

ggplot(df3) +
  geom_density(aes(x = -log10(value), fill = type), alpha = 0.5) +
  labs(x = "-log10(p-value)", y = "Density", fill = "") +
  theme_light() +
  theme(legend.position = c(0.9, 0.85))
```

**Note:** Strictly speaking, we *should* have performed all p-value-based analyses (filtering and downstream steps) based on the adjusted p-values.
